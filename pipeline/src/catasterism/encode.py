...
from .constants import ABS_MAG_MAX, ABS_MAG_MIN, COLOUR_LUT_SIZE, MAGNITUDE_BITS
...
def encode(table: pa.Table, release: Release) -> EncodeResult:
    ...
    return EncodeResult(records.tobytes(), manifest, dropped, stats)
...
def decode(data: bytes, manifest: dict) -> dict[str, np.ndarray]:
    ...
    r = np.frombuffer(data, dtype=np.uint32).reshape(-1, 4)
    packed = r[:, 3]
    return {
